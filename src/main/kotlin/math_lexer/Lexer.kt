...
class Lexer(private val input: String) {
    ...
    fun nextToken(): Token =
        if (currentParsePosition >= input.length) Token(TokenType.EndOfInput, "", currentParseLine, currentParseColumn)
        else {
            skipWhiteSpacesAndNewLines()
            recognizeLexem(input[currentParsePosition])
        }
    ...
    fun recognizeNumber(): Token {
        ...
        val (recognized, number) = fsm.invoke(fsmInput)
